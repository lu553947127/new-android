...
public class PaySecureHelper {
            ...
            getCode(ctx, coreManager, key, t -> {
                ...
                getRsaPrivateKey(ctx, coreManager, key, t -> {
                    ...
                    public void apply(byte[] privateKey) {
                        ...
                        String mac = RSA.signBase64(FLYAppConfig.apiKey + coreManager.getSelf().getUserId() + UserSp.getInstance(ctx).getAccessToken() + valueString + time + payPasswordMd5, privateKey);
    ...
    private static void getCode(Context ctx, CoreManager coreManager, byte[] key, Function<Throwable> onError, Function2<String, String> onSuccess) {
        ...
        String mac = MAC.encodeBase64((FLYAppConfig.apiKey + coreManager.getSelf().getUserId() + UserSp.getInstance(ctx).getAccessToken() + salt).getBytes(), payPasswordMd5);
