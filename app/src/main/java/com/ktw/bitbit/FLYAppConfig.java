...
public class FLYAppConfig {
    ...
    public static final String BroadcastReceiverClass = FLYBroadcastReceiver.class.getName();
...
//    public static String HOST = "http://192.168.0.86:8092/";// Socket IM 服务器入口
    public static String HOST = "https://admin.bitbitchat.top/";
//    public static String HOST = "http://180.215.115.175:8092/";
    ...
    //表情包
    public String API_FACE_LIST;//获取所有表情包
    public String API_FACE_COLLECT_LIST;//获取所有收藏的表情包
    public String API_FACE_COLLECT_ADD;//收藏表情包以及单个表情
    public String API_FACE_NAME_DELETE;//根据名字删除整个表情包
    public String API_FACE_LIST_NAME_GET;//根据名字获取表情包详情
    public String API_FACE_ID_DELETE;//根据名字删除单个表情
    ...
    public String ROOM_JOIN2;// 加入群组
    public String ROOM_MEMBER_UPDATE;// 添加成员
    public String ROOM_MEMBER_UPDATE2;// 添加成员2
    ...
    public String ROBOT_AUTO_ANSWER;//机器人自动回答问题列表
    public String VIEW_QUESTION_LIST;//问题列表
    public String FREEZE_ACCOUNT;//冻结账号
    ...
    //==================================bitbitstart===============================================
    public String FIRST_LOGIN;//唯一验证登录
    public String FIRST_REGISTER;//唯一验证注册
    public String ALL_GROUP;//获取全部置换群
    public String IS_JOIN_GROUP;//判断当前用户是否加入某个群聊
    public String ADMIN_USER;//获取创建置换群的群主
    public String SPRAY_PERMISSION;//浪花币种权限
    public String IS_ALREADY_GROUP;//验证用户是否已加过置换群
    //==================================bitbitend==================================================

    public String GET_RECHARGE_ORDER;//第三方充值订单
    public static final String CNPPAY_INITPAY = "http://p.amghggo.cn/cnpPay/initPay";   //聚合支付 单笔订单支付请求
    /* 登出地址 */
    ...
    public static boolean isShiku() {
        return !TextUtils.isEmpty(apiKey)
                ...
                && Objects.equals(BuildConfig.APPLICATION_ID, BuildConfig.APPLICATION_ID);
    }
    ...
    public static String readConfigUrl(Context ctx) {
        ...
        if (TextUtils.isEmpty(appUrl)) {// 未手动输入过服务器配置，使用默认地址
            appUrl = FLYAppConfig.CONFIG_URL;
            // 保存默认地址，下次使用，
     ...
     * 会改变的配置
     **/
    public static FLYAppConfig initConfig(ConfigBean configBean) {
        FLYAppConfig config = new FLYAppConfig();
        /* 1、Api 的服务器地址 */
        ...
        FLYApplication.IS_OPEN_RECEIPT = isOpenReceipt == 1;
    ...
    private static void initApiUrls(FLYAppConfig config) {
        ...
        config.API_FACE_LIST = apiUrl + "face/list";
        config.API_FACE_COLLECT_LIST = apiUrl + "faceClollect/list";
        config.API_FACE_COLLECT_ADD = apiUrl + "faceClollect/addFaceClollect";
        ...
        //聚合支付
        config.GET_RECHARGE_ORDER = apiUrl + "user/recharge/getSign";   //聚合支付 生成订单号
        //机器人
        config.ROBOT_AUTO_ANSWER = apiUrl + "api/AutoAnswer/getIssue";
        //获取问题列表
        config.VIEW_QUESTION_LIST = apiUrl + "api/edit/getListByType";
        //冻结账号
        config.FREEZE_ACCOUNT = apiUrl + "api/Freeze/apply";
        //注销账号
        config.CANCEL_ACCOUNT = apiUrl + "user/cancel";
        //问题反馈
        config.QUESTION_FEEDBACK = apiUrl + "api/Complaint/submit";
        //获取单条信息
        ...
        //===============================bitbitstart===============================================================
        config.FIRST_LOGIN = apiUrl + "adminConsole/adminCheckUser";//唯一验证登录
        config.FIRST_REGISTER = apiUrl + "adminConsole/adminUpdateUser";//唯一验证注册
        config.ALL_GROUP = apiUrl + "adminConsole/adminUserGroup";//获取全部置换群
        config.IS_JOIN_GROUP = apiUrl + "adminConsole/adminCheckUserGroup";//判断当前用户是否加入某个群聊
        config.ADMIN_USER = apiUrl + "adminConsole/adminUser";//获取创建置换群的群主
        config.SPRAY_PERMISSION = apiUrl + "adminConsole/flower";//浪花币种权限
        config.IS_ALREADY_GROUP = apiUrl + "adminConsole/adminUserGroupList";//验证用户是否已加过置换群
        //===============================bitbitend===============================================================
    }
    ...
    private static void initOthersUrls(FLYAppConfig config) {
        ...
        LogUtils.log("config.AVATAR_UPLOAD_URL" + config.AVATAR_UPLOAD_URL);
    }
}
