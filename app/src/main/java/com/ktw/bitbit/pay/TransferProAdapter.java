...
import com.ktw.bitbit.bean.TransferRecord;
import com.ktw.bitbit.ui.base.BaseRecViewHolder;
...
public class TransferProAdapter extends RecyclerView.Adapter {
    ...
    class HeadViewHolder extends BaseRecViewHolder {
        ...
        public void onBind(List<TransferRecord.DataBean.PageDataBean> list, int position) {
            ...
            if ((month - 1) == Calendar.getInstance().get(Calendar.MONTH)) {
                tv_month.setText(FLYApplication.getContext().getString(R.string.this_month));
            } else {
                tv_month.setText(FLYApplication.getContext().getString(R.string.month, month));
            }
    ...
    class TransferViewHolder extends BaseRecViewHolder {
        ...
        public void onBind(List<TransferRecord.DataBean.PageDataBean> list, int position) {
            ...
            tv_receipt.setText(list.get(position).getStatus() == 1 ? FLYApplication.getContext().getResources().getString(R.string.jiao_yi_c_g) :
                    FLYApplication.getContext().getResources().getString(R.string.jiao_yi_s_b));//状态
            ...
            switch (list.get(position).getType()) {
                ...
                case 10:
                case 12:
                    tv_balance.setTextColor(FLYApplication.getContext().getResources().getColor(R.color.records_of_consumption));
                    tv_balance.setText(String.valueOf("-" + list.get(position).getMoney()));//支出金额
                    break;
                default:
                    tv_balance.setTextColor(FLYApplication.getContext().getResources().getColor(R.color.records));
