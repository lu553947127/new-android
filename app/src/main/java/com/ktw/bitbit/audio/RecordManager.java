...
import com.ktw.bitbit.audio.record.AmrEncoder;
import com.ktw.bitbit.audio.record.AudioFileUtils;
import com.ktw.bitbit.audio.record.AudioRecorder;
...
public class RecordManager {
    ...
    public void startRecord() {
        ...
        try {
            requestAudioFocus();
            notifyStartLoading();
            AudioFileUtils.setFolder(FLYApplication.getContext().getFilesDir().getAbsolutePath());
    ...
    public synchronized String stop() {
        if (mr != null) {
            ...
            if (delay <= 500) {
                ...
                LogUtils.e("RecordLog", "stop name-->" + name);
                LogUtils.e("RecordLog", "stop startTime-->" + startTime);
                LogUtils.e("RecordLog", "stop delay-->" + delay);
                notifyFinish(name);
            }
        } else {
            notifyCancal();
    ...
    private AudioManager getAudioManager() {
        if (mAudioManager == null) {
            synchronized (this) {
                if (mAudioManager == null) {
                    mAudioManager = (AudioManager) FLYApplication.getContext()
