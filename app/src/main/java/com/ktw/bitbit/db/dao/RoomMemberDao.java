...
import com.ktw.bitbit.db.SQLiteRawUtil2;
import com.ktw.bitbit.db.UnlimitDaoManager;
...
public class RoomMemberDao {
     ...
     * 获取表内某条数据
     */
    public List<RoomMember> queryRoomMaster(String roomId) {
        Dao<RoomMember, Integer> dao = getDao(roomId);
        if (dao == null) {
            return null;
        }
        QueryBuilder<RoomMember, Integer> builder = dao.queryBuilder();
        List<RoomMember> mRoomMembers = null;
        try {
            if (!TextUtils.isEmpty(roomId)) {
                builder.where().eq("roomId", roomId).and().between("role", 1, 2);
            }
            mRoomMembers = dao.query(builder.prepare());
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return mRoomMembers;
    ...
    public String getRoomRemarkName(String roomJid, String userId) {
        Friend friend = FriendDao.getInstance().getFriend(CoreManager.requireSelf(FLYApplication.getContext()).getUserId(), roomJid);
        if (friend != null) {
            RoomMember mMember = getSingleRoomMember(friend.getRoomId(), CoreManager.requireSelf(FLYApplication.getContext()).getUserId());
