...
public class VideoFileDao {
    ...
    private VideoFileDao() {
        try {
            dao = DaoManager
                    .createDao(OpenHelperManager.getHelper(FLYApplication.getInstance(), SQLiteHelper.class).getConnectionSource(), VideoFile.class);
    ...
    public void deleteUserVideoHistory(String ownerId) {
        try {
            DeleteBuilder<VideoFile, Integer> builder = dao.deleteBuilder();
            builder.where().eq("ownerId", ownerId);
