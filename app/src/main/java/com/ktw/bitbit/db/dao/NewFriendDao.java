...
public class NewFriendDao {
    ...
    private NewFriendDao() {
        try {
            newFriendDao = DaoManager.createDao(OpenHelperManager.getHelper(FLYApplication.getInstance(), SQLiteHelper.class).getConnectionSource(),
    ...
    public void changeNewFriendState(String friendId, int state) {
        UpdateBuilder<NewFriendMessage, Integer> builder = newFriendDao.updateBuilder();
        String myId = CoreManager.requireSelf(FLYApplication.getInstance()).getUserId();
        try {
...
*/
    ...
    private void createFriendAndSetStatus(final NewFriendMessage newFriend, final int status) {
        ...
        HttpUtils.get().url(CoreManager.requireConfig(FLYApplication.getInstance()).USER_GET_URL)
                .params(params)
                .build()
                    ...
                    public void onResponse(ObjectResult<User> result) {
                        if (result.getResultCode() == 1 && result.getData() != null) {
                            ...
                            if (user.getUserType() == 2) {// 公众号
                                FriendDao.getInstance().updateFriendStatus(CoreManager.requireSelf(FLYApplication.getContext()).getUserId(),
    ...
    public void addFriendOperating(String userId, String nickName, String remarkName) {
        ...
        NewFriendMessage message = NewFriendMessage.createLocalMessage(
                CoreManager.requireSelf(FLYApplication.getInstance()), XmppMessage.TYPE_FRIEND, null, userId, nickName);
        ...
        addChatMessage.setContent(FLYApplication.getInstance().getString(R.string.add_friend) + ":" + nickName);
        ...
        FriendDao.getInstance().updateFriendContent(mLoginUserId, userId,
                FLYApplication.getInstance().getString(R.string.be_friendand_chat), XmppMessage.TYPE_TEXT, TimeUtils.sk_time_current_time());
        ListenerManager.getInstance().notifyNewFriend(mLoginUserId, message, true);
        CardcastUiUpdateUtil.broadcastUpdateUi(FLYApplication.getContext());
    }

    public void deleteNewFriendMessage(String ownerId){
        try {
