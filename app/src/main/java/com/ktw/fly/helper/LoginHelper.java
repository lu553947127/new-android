...
public class LoginHelper {
    ...
    public static final String ACTION_NEED_UPDATE = FLYAppConfig.sPackageName + ".action.need_update";
    public static final String ACTION_LOGIN_GIVE_UP = FLYAppConfig.sPackageName + ".action.login_give_up";// 在下载资料的时候，没下载完就放弃登陆了
    ...
    public static void broadcastLogin(Context context) {
        ...
        if (context == null) {// 防止传入Context对象的界面被销毁导致context为空
            context = FLYApplication.getContext();
        }
    ...
    public static int prepareUser(Context context, CoreManager coreManager) {
        ...
        FLYApplication.getInstance().mUserStatus = userStatus;
    ...
    public static boolean setLoginUser(Context context, CoreManager coreManager, String account,
        ...
        userStatus.accessToken = result.getData().getAccessToken();
        ...
        FLYApplication.getInstance().mUserStatusChecked = true;
        FLYApplication.getInstance().mUserStatus = STATUS_USER_VALIDATION;
        coreManager.setSelf(user);
