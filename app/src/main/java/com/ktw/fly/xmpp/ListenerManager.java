...
import com.ktw.fly.xmpp.listener.MucListener;
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                if (!hasRead && isPreRead) {
                    ...
                    MsgBroadcast.broadcastMsgNumUpdateNewFriend(FLYApplication.getInstance());
                }
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                for (int i = mChatMessageListeners.size() - 1; i >= 0; i--) {
                    ...
                    tempMessage.setMessageState(message.getMessageState());
                ...
                String selfId = CoreManager.requireSelf(FLYApplication.getInstance()).getUserId();
                if (isGroupMsg) {
                    ...
                    if (!hasRead
                        ...
                        if (isRepeatFriend) {// 同一个群组不止一个，需要更新
                            FLYApplication.getContext().sendBroadcast(new Intent(Constants.UPDATE_ROOM));
                        }
                        // 发送广播更新总未读消息数量
                ...
                MsgBroadcast.broadcastMsgUiUpdateSingle(FLYApplication.getInstance(), fromUserId);
            }
        });
    }
