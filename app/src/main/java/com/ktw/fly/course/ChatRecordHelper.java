...
public class ChatRecordHelper {
    ...
    public void iniText(TextView tvRecord, ChatMessage message) {
        ...
        if (mState == STATE_UN_RECORD) {
            ...
            tvRecord.setText(FLYApplication.getInstance().getString(R.string.record_course));
        } else {
            rid = R.drawable.stoped;
            tvRecord.setText(FLYApplication.getInstance().getString(R.string.save_course));
        }
    }
    ...
    public void stop(ChatMessage chatMessage, String toUserId) {
        if (mState == STATE_RECORDING && chatMessage != null) {
            List<ChatMessage> chatMessages = ChatMessageDao.getInstance().getCourseChatMessage(CoreManager.requireSelf(FLYApplication.getInstance()).getUserId(), toUserId,
            ...
            for (int i = 0; i < chatMessages.size(); i++) {
                if (chatMessages.get(i).isMySend() && (chatMessages.get(i).getType() == XmppMessage.TYPE_TEXT
                        ...
                        || chatMessages.get(i).getType() == XmppMessage.TYPE_FILE)) {// 只录制自己发送的文本消息、语音、图片、视频、文件
