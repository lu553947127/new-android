...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ViewHolder> {
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void apply(Item item) {
            ...
            ColorStateList tabColor = SkinUtils.getSkin(FLYApplication.getInstance()).getTabColorState();
            cbSelect.setOnCheckedChangeListener(null);
            cbSelect.setChecked(item.selected);
            //不点击时 初始化的时候有效
            if (cbSelect.isChecked()) {
                Drawable drawable = FLYApplication.getInstance().getResources().getDrawable(R.drawable.sel_check_wx2);
                drawable = DrawableCompat.wrap(drawable);
                DrawableCompat.setTintList(drawable, tabColor);
                cbSelect.setButtonDrawable(drawable);
            } else {
                cbSelect.setChecked(false);
                cbSelect.setButtonDrawable(FLYApplication.getInstance().getResources().getDrawable(R.drawable.sel_nor_wx2));
            }
