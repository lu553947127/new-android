...
import com.ktw.fly.ui.message.HandleQRCodeScanUtil;
...
public class MultiImagePreviewActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            mImages = (ArrayList<String>) getIntent().getSerializableExtra(FLYAppConstant.EXTRA_IMAGES);
            for (String mImage : mImages) {
                Log.e("MultiImage", "dateSource:" + mImage);
            }
            mPosition = getIntent().getIntExtra(FLYAppConstant.EXTRA_POSITION, 0);
            mChangeSelected = getIntent().getBooleanExtra(FLYAppConstant.EXTRA_CHANGE_SELECTED, false);
        }
        if (mImages == null) {
    ...
    private void doFinish() {
        if (mChangeSelected) {
            ...
            intent.putExtra(FLYAppConstant.EXTRA_IMAGES, resultImages);
    ...
    private void register() {
        ...
        filter.addAction(com.ktw.fly.broadcast.OtherBroadcast.singledown);
        filter.addAction(com.ktw.fly.broadcast.OtherBroadcast.longpress);
    ...
    class My_BroadcastReceivers extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(com.ktw.fly.broadcast.OtherBroadcast.singledown)) {
                doFinish();
            } else if (intent.getAction().equals(com.ktw.fly.broadcast.OtherBroadcast.longpress)) {
                // 长按屏幕，弹出菜单
    ...
    class ImagesAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            // copy from com.ktw.fly.ui.tool.SingleImagePreviewActivity.initView
