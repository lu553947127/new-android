package com.ktw.fly.ui.mucfile;
...
public class UploadingHelper {
    ...
    public static void upfile(String accessToken, String userId, final File file, final OnUpFileListener onUpFileListener) {
        try {
            if (!file.exists()) {
                onUpFileListener.onFailure(FLYApplication.getContext().getString(R.string.alert_not_have_file), file.getAbsolutePath());
            }

            if (TextUtils.isEmpty(userId)) {
                onUpFileListener.onFailure(FLYApplication.getContext().getString(R.string.tip_user_id_empty), file.getAbsolutePath());
            }

            if (onUpFileListener == null) {
                onUpFileListener.onFailure(FLYApplication.getContext().getString(R.string.tip_upload_listener_empty), file.getAbsolutePath());
            }
                ...
                public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {//数据发送成功，并且有返回值<不是上传成功。。。>
                    ...
                    if (arg0 == 200) {
                        ...
                        if (result == null || result.getResultCode() != Result.CODE_SUCCESS || result.getData() == null
                            ...
                            onUpFileListener.onFailure(FLYApplication.getContext().getString(R.string.upload_failed), file.getAbsolutePath());
                        } else {
                    ...
                    if (TextUtils.isEmpty(url)) {
                        ...
                        onUpFileListener.onFailure(FLYApplication.getContext().getString(R.string.tip_upload_result_empty), file.getAbsolutePath());
                    } else {
