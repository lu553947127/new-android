...
import com.ktw.fly.audio.IMRecordController;
import com.ktw.fly.audio.RecordListener;
...
import com.ktw.fly.course.ChatRecordHelper;
...
import com.ktw.fly.util.InputManager;
import com.ktw.fly.util.PreferenceUtils;
import com.ktw.fly.util.SmileyParser;
import com.ktw.fly.util.UiUtils;
import com.ktw.fly.util.filter.EmojiInputFilter;
import com.ktw.fly.util.input.KeyboardPanelUtil;
...
//                        intent.putExtra(EXTRA_URL, HttpUtils.get().url(url).buildUrl(true, false));
//                        getContext().startActivity(intent);
//                    }
                    //跳转10005页面
                    Friend friend = FriendDao.getInstance().getFriend(CoreManager.getInstance(mContext).getSelf().getUserId(), Friend.ID_SYSTEM_NOTIFICATION);
                    Intent serviceIntent = new Intent(mContext, ChatActivity.class);
                    serviceIntent.putExtra(ChatActivity.FRIEND, friend);
...
//                    }
                    //跳转问题列表页面
                    mContext.startActivity(new Intent(mContext, ViewQuestionActivity.class));
                    break;
    ...
    private void changeChatFaceView(boolean show) {
        ...
        if (show) {
            if (mChatFaceView == null) {
                mChatFaceView = (ChatFaceView2) mChatFaceViewStub.inflate();
                mChatFaceView.setEmotionClickListener(new ChatFaceView2.EmotionClickListener() {
                    @Override
                    ...
                    @Override
                    public void onCustomEmotClick(String name) {
                        if (mBottomListener != null) {
                            mBottomListener.sendCustomEmot(name);
                        }

                    }

                    @Override
                    public void onEmotPackageClick(String emot) {
                        if (mBottomListener != null) {
                            mBottomListener.sendEmotPackage(emot);
                        }
                    }
                });
            }
...
//                        showKeyboard();
    ...
    public void fillRoomMenu(List<PublicMenu> datas) {
        if (lLTextMenu == null) {
            ...
            // 切换到主菜单
            flTextMenu = lLTextMenu.findViewById(R.id.fl_text_meun);
            flTextMenu.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    showChatBottom();
                }
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!TextUtils.isEmpty(bean.getMenuId())) {
                    // 域名+menuId+token
                    url = HttpUtils.get().url(CoreManager.requireConfig(FLYApplication.getInstance()).apiUrl + bean.getMenuId())
                            .buildUrl(true, false);
    ...
    public interface ChatBottomListener {
        ...
        void sendCustomEmot(String name);

        void sendEmotPackage(String name);
